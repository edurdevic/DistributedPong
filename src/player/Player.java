      ...
      while (true){
        ...
        try{
            ...
            try {
                ...
                while (true){
                    //>> Pensa dove devi andare
                    //>> vai a sx o dx
                    if (playingD.getxB()>playingD.getMyPosition()){
                        goRight();
                    } else {
                        goLeft();
                    }
...
//                playingOut.close();
    ...
    public PlaygroundData FindPlayground() throws Exception{
        ...
        if (rowN == 0) { throw new Exception("Non ci sono Playground disponibili."); }
        playgrounds = new TreeSet<PlaygroundData>();
        for (int i=1; i<rowN; i++){                     //Scorro tutte le righe della risposta
            ...
            if (dataResponse.length >= 7){
                try {
                    playgrounds.add(new PlaygroundData(
    ...
    public PlaygroundData FindPlaygroundManually() throws Exception{
        return new PlaygroundData(InetAddress.getLocalHost(), 7960, 7961, 800, 600, 1.1, 0.2);
    }
        ...
        if (response.startsWith("OK")){
            ...
            if (dataResponse.length == 1) {
                ...
                try {
                    ...
                    log.debugMessage("Cavoli, non posso istanziare PlayingData " + e.getMessage());
                }
            }
            return true;
